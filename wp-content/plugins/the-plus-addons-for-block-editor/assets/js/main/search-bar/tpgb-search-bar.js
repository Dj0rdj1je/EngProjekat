    ...
    $('.tpgb-search-bar').each( function() {
		...
		let OverlayBg = main[0].querySelectorAll('.tpgb-rental-overlay');
            ...
            $(main[0]).on("change keyup", function(e){
                ...
                if( (EvnetType == 'keyup' && serText && serText.length >= $Searhlimit) || (EvnetType == 'change' && (post && SpecialCTP == 0) || ((post && serText && serText.length >= $Searhlimit) || (post=='' && serText.length >= $Searhlimit)) || (cat && SpecialCTP == 0) || (cat && serText && serText.length >= $Searhlimit) || (cat=='' && serText && serText.length >= $Searhlimit)) ){

                    var tCloseBtn = main[0].querySelector('.tpgb-close-btn'), tajaxload = main[0].querySelector('.tpgb-ajx-loading'), serInArrow = main[0].querySelector('.tpgb-dd-icon');
                    if(tCloseBtn){
                        tCloseBtn.style.cssText = "display:none";
                    }
                    if(main[0].classList.contains('tpgb-ser-input-dis')){
                        serInArrow.style.cssText = "display:none";
                    }
                    if(tajaxload){
                        tajaxload.style.cssText = "display:flex";
                    }
            ...
            function tpgb_block_searchbar(search) {
                ...
                $.ajax({
                    ...
                    setTimeout(function(){ 
                        var tCloseBtn = main[0].querySelector('.tpgb-close-btn'), tajaxload = main[0].querySelector('.tpgb-ajx-loading');
                        if(tajaxload){
        ...
        var tp_Close_result = function() {
            let Area = main[0].querySelector('.tpgb-search-area'),
                ...
                serInArrow = main[0].querySelector('.tpgb-dd-icon');
